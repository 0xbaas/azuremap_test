...
# the Entra product subtree (Core\Entra). No Core\Azure: EntraMap performs no
# subscription discovery or ARM scanning.
$alreadyLoaded = @("State.ps1", "Logging.ps1", "Config.ps1", "Exclusions.ps1")
foreach ($coreDir in @("$scriptRoot\Core", "$scriptRoot\Core\Entra")) {
...
try {
    ...
    # 6.5 Assessment plan: how many registered checks are relevant vs filtered.
    ...
    # 8.5 Capability modeling is an AzureMap feature for now: Build-CapabilityModel
    #     lives in Core\Azure and is absent here, so this step no-ops gracefully.
