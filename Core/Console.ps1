...
function Get-CheckDomain {
    ...
    if ("$($Check.Category)" -eq 'Entra') { return 'Identity' }
...
function Get-CheckDomainSortIndex {
    ...
    [CmdletBinding()]
    param([Parameter(Mandatory)][object]$Check)
    $idx = [Array]::IndexOf($script:CheckDomainOrder, (Get-CheckDomain -Check $Check))
    if ($idx -lt 0) { return $script:CheckDomainOrder.Count }
    return $idx
}
...
function Show-AssessmentPlan {
    ...
    .SYNOPSIS
        ...
        filter, blocked by missing modules, or not in scope (footprint-proven).
    #>
    [CmdletBinding()]
    param(
        [switch]$SkipEntra,
    ...
    $planned = 0; $relevant = 0; $modeSkipped = 0; $filtered = 0; $modMissing = 0; $notApp = 0; $dpGated = 0
    ...
    foreach ($c in @(Get-AuditChecks)) {
        ...
        $applic = Get-CheckApplicability -Check $c
    ...
    if ($script:State.Config.DetailedSummary) {
        ...
        if ($exec.Count -eq 0) {
            ...
            $ordered = @($exec | Sort-Object {
                $idx = [Array]::IndexOf($script:CheckDomainOrder, (Get-CheckDomain -Check $_))
                if ($idx -lt 0) { $script:CheckDomainOrder.Count } else { $idx }
            }, CheckId)
            $lastDomain = $null
