#==============================================================================
# AzureMap v2 - Products/EntraMap/Core/Graph.ps1
...
function Get-GraphTokenScopeInfo {
    <#
    .SYNOPSIS
        Decodes the cached Graph JWT once per run and caches the granted scopes.
    .DESCRIPTION
        Extracts 'roles' (application) and 'scp' (delegated) claims from the
        cached token and caches the result in $script:State.Cache.Graph so the
        assessment plan and the per-check permission gate never decode twice.
        DecodeSucceeded is $false when there is no token or the payload is not
        a decodable JWT (e.g. a mocked/opaque token): callers MUST treat the
        permission state as unknown and fail open (never invent a limited
        count, never gate a check off) in that case. The token value itself is
        never logged.
    .PARAMETER ForceRefresh
        Ignore the cached decode and decode the current token again.
    .OUTPUTS
        [PSCustomObject] @{ GrantedScopes = [string[]]; DecodeSucceeded = [bool] }
    #>
    [CmdletBinding()]
    param(
        [switch]$ForceRefresh
    ...
    $cacheKey = 'TokenScopeInfo'
    if (-not $ForceRefresh -and
        $script:State.Cache.Graph -and
        $script:State.Cache.Graph.ContainsKey($cacheKey)) {
        return $script:State.Cache.Graph[$cacheKey]
    }

    $result = [PSCustomObject]@{ GrantedScopes = @(); DecodeSucceeded = $false }
    ...
    if ($token) {
        try {
            ...
            if ($parts.Count -ge 2) {
                ...
                if ($claims.roles) {
                    foreach ($r in @($claims.roles)) { $granted.Add([string]$r) }
                }
                if ($claims.scp) {
                    foreach ($s in ([string]$claims.scp -split ' ')) {
                        if ($s) { $granted.Add($s) }
                    }
                }
                $result.GrantedScopes   = $granted.ToArray()
                $result.DecodeSucceeded = $true
            }
        }
        catch {
            Write-AuditLog -Message "Graph token scope decode failed; permission state unknown: $($_.Exception.Message)" -Level DEBUG
        }
    }

    if ($script:State.Cache.Graph) {
        $script:State.Cache.Graph[$cacheKey] = $result
    }
    return $result
}
