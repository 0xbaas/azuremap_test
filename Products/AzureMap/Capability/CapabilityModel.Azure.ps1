#==============================================================================
# AzureMap v2 - Core/Azure/CapabilityModel.Azure.ps1
# Phase B2: Azure capability / attack-path modeling (READ-ONLY MODELING
# ONLY): Azure role/action constants, the key-list action matcher, the
# Azure input readers (inventory cache, RBAC cache), the seven Azure
# insight builders and Build-CapabilityModel (extracted from
# Core/CapabilityModel.ps1). Shared primitives live in Core/Capability.ps1.
#
# Builds a capability graph (nodes/edges) plus grouped capability insights
# from data that is ALREADY collected during the run: finding evidence rows
# ($script:State.Results), the in-memory inventory cache
# ($script:State.Cache.ResourceLists, Core/Azure/InventoryCache.ps1), the
# RBAC assignment cache ($script:State.Cache.RBACAssignments) and the
# environment footprint ($script:State.Footprint).
#
