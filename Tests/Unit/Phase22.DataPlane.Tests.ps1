#==============================================================================
# AzureMap v2 - Tests/Unit/Phase22.DataPlane.Tests.ps1
...
Describe "Phase B3 switch surface" {
    ...
    It "azuremap.ps1 declares -IncludeDataPlane" {
        $src = Get-Content (Join-Path $projectRoot 'azuremap.ps1') -Raw
    ...
    It "STORAGE-004 and KEYVAULT-003 are registered as data-plane checks" {
        $storage  = Get-Content (Join-Path $projectRoot 'Checks\Azure\Storage.ps1')  -Raw
        $keyvault = Get-Content (Join-Path $projectRoot 'Checks\Azure\KeyVault.ps1') -Raw
...
Describe "Unattended-run hardening (no prompts, no module warning leaks)" {
    ...
    BeforeAll {
        $script:entrypoint = Get-Content (Join-Path $projectRoot 'azuremap.ps1') -Raw
    }
    ...
    It "Write-Finding never prints blocks for VerboseOutput alone (only -ShowFindings/-ShowRemediation)" {
        $registry = Get-Content (Join-Path $projectRoot 'Core\CheckRegistry.ps1') -Raw
