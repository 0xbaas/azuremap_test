...
It scans Azure subscriptions for misconfigurations, excessive permissions, and compliance gaps, models capability / attack-path risk from the results, and exports findings to CSV, JSON, and HTML. It never requests a Microsoft Graph token — for Entra ID tenant audits, use [EntraMap](EntraMap.md).
...
- [EntraMap](EntraMap.md) — the Entra ID (Microsoft Graph) product
