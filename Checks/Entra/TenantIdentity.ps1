...
function Test-DormantServicePrincipals {
    ...
    if (@($Subscriptions).Count -eq 0) {
        Write-Finding -Severity "INFO" -Status "NotEvaluated" `
                      -Message "Service principals without password/key credentials identified tenant-wide; RBAC assignment correlation not evaluated (no Azure subscription scope)" `
